...
// Per-tile terrain masks for the outdoor regions (32x22, 16px tiles).
// '#' = blocked terrain, '~' = pond surface, '.' = walkable ground.
//
// This is the single runtime source for terrain collision. Water is unioned
// separately from WORLD_WATER_ROWS (fishing reads that too) and building
// footprints stay as rects derived from villageLayout so buildings can move.
//
// Do not hand-edit: regenerate with `node scripts/build-terrain-masks.mjs`,
// which holds the rect terrain (borders, cliff shelves, forest blocks) and the
// hand-checked organic cells, then unions them into these rows.
