...
export class FarmVillageScene extends Phaser.Scene {
  ...
  private syncCameraViewport(snapshot: PhaserWorldSnapshot) {
    ...
    const zoom = Math.max(0.01, snapshot.camera.zoom);
    const camera = this.cameras.main;
    camera.setViewport(0, 0, canvasWidth, canvasHeight);
    camera.setOrigin(0, 0);
    camera.setZoom(zoom);
    camera.setScroll(-snapshot.camera.left / zoom, -snapshot.camera.top / zoom);
    ...
    canvas.dataset.phaserSnapshotLeft = String(snapshot.camera.left);
    canvas.dataset.phaserSnapshotTop = String(snapshot.camera.top);
  }
